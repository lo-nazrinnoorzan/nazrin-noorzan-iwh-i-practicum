...
app.get('/', async (req, res) => {
    const customObjects = 'https://api.hubspot.com/crm/v3/objects/2-222991858?properties=pet_name&properties=pet_type&properties=pet_age';
    const headers = {
    ...
    try {
        ...
        res.render('homepage', { title: 'Pets Custom Objects | Integrating With HubSpot I Practicum', data });      
    } catch (error) {
        console.error(error);
    }
});
...
app.post('/update-cobj', async (req, res) => {
    ...
    const createCustomObject = 'https://api.hubspot.com/crm/v3/objects/2-222991858';
    const headers = {
        Authorization: `Bearer ${PRIVATE_APP_ACCESS}`,
        'Content-Type': 'application/json'
    };
    ...
    try { 
        await axios.post(createCustomObject, newPet, { headers } );
        res.redirect('/');
    } catch(err) {
        console.error(err);
    }
